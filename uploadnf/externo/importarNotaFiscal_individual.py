...
#**************************************************************************************************************
#nr_pedido = 24243
nr_pedido = input('digite o número do pedido: ')
#**************************************************************************************************************
n = 1
lista_pdd_bling = []
#while True:
#url = f"https://bling.com.br/Api/v2/pedidos/page={n}/json/"
url = f"https://bling.com.br/Api/v2/pedido/{nr_pedido}/json/"
...
payload = {
    "apikey": BLING_SECRET_KEY,
    "filters": f"idSituacao[{situacao_id}]",
}
...
class NotaFiscalSaida(object):
    ...
    def __init__(self, codNfSerie, numero, serie, numero_nf, dataEmissao_nf, valorNota, chaveAcesso, qtvol, cgc_transp):
...
conj = (pedd_bling_num )  # Um conjunto de pedidos do bling (-) conjunto de pdd db itotal)
...
for u in dados['retorno']['pedidos']:
    ...
    if 'transporte' in pedido:
        ...
        if 'cnpj' in u['pedido']['transporte']:
