...
data_ago = datetime.now()-timedelta(hours=24)
#############################################################################################################
...
#querystring = {"updated_at__gt": new_data} ** updated_at__gt (__gt é igual a data maior que:)
...
#***********************************************************************
#nrpdd = 24243
nrpdd = input('digite o número do pedido: ')
#***********************************************************************
#def list_pedidos(page=1):
#url = f'http://34.194.18.67/pedidos/pedidos/page={page}'
url = f'http://mbp.f3system.com.br/pedidos/pedidos/{nrpdd}/'
...
pedidos = requests.get(url, headers=headers)
data = json.loads(pedidos.text)


print("printa data ===>", data)


#try:
#pagina = data['itens']
#print("achei a pagina ", pagina)
#pag += 1
...
"""
for item in pagina:
    all_pedidos['results'].append(item)
    #except KeyError:
    #    break """
