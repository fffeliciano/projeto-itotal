...
def processa_imagem(medidas, im, medidas1, medidas2):

    imRt = im.crop((medidas))
    imRt1 = im.crop((medidas1))
  #  imRt2 = im.crop((medidas2))
    #ubuntu
  ...
  #  fileNameImg2 = 'etiquetas/nameImagemNova_nota.png'   # retangulo codigo rastreamento

    imRt.save(fileNameImg,"PNG")
    imRt1.save(fileNameImg1,"PNG")
  #  imRt2.save(fileNameImg1,"PNG")
    ...
    if not texto :
        ...
        for n in texto:
            ...
            if title1 == "Venda:":
                ...
                print("text___>>", text)
                texto = text.splitlines()
                print("texto da Etiqueta ?? ------>", texto[0][3:])
                print(type(texto[0][3:]))


                numped_mktpl_origin = texto[0][3:].replace(" ","")

                print("numped_mktpl_origin :====>", numped_mktpl_origin)
                #print("len numped_mktpl_origin :====>", len(numped_mktpl_origin))
                #print(" vai quebrar aqui!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                #exit()
            ...
            numped_mktpl_base = numped_mktpl_origin.strip().replace(" ","")
            ...
            if len(numped_mktpl_base) == 16:
            ...
            elif len(numped_mktpl_origin) == 5:
                #numped_mktpl = n #if n[0:2].find('0') > 0 else n[0:2].replace('0','O') + n[2:13]
                ...
                #print('número da Nota Fiscal',w)
                #y = w[0]
                print("Numero da NOTA FISCAL : ===> :", numped_mktpl_origin)
                #print("y ===>", y['numnf'])
                
                
                novoNome = numped_mktpl_origin
                newName1 = "etiquetas/" + str(novoNome)
                newName3 = str(novoNome)
                print('novoNome ====>', novoNome)
                print('newName1 ====>', newName1)
...
def lerEtiquetas_ml(name_file):
    ...
    for i, image in enumerate(images):
        ...
        medidas2 = [1130,2300,1500,2370]
        print("# Etiqueta 1")
        print("medidas :", medidas)
        print("medidas1 :", medidas1)
        print("medidas2 :", medidas2)

        testar = processa_imagem(medidas, im, medidas1, medidas2)
...
z = lerEtiquetas_ml("xml/01112022ML2-testePrincipal.pdf")
